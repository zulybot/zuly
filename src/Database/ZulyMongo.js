...
class Database extends EventEmitter {
		...
		if (this.firstConnect) {
			...
			this.db
				...
				.on('disconnect', () => {
					...
					if (!this.forcedDisconnect && this.forceConnection) {Mongo.connect(this.uri, this.options);}
				});
		}
   ...
   */
	async get (key) {
		const obj = await this.model?.findOne({ name: key }).exec();
   ...
   */
	async getExec (key) {
		const exec = await this.model?.findOne({ name: key }).exec();
		return exec;
	}
   ...
   */
	async set (key, value) {
		...
		if (obj) {
			...
			const new_obj = new this.model({ name: key, value });
			return new_obj.save();
		}
	}
