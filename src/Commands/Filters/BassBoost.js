	...
	async run (ctx) {
		...
		if (!player) {
			...
			if (player.bassboost === false) {
				...
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.ativado.replace('%f', 'bassboost')}`);
			}
			if (player.bassboost === true) {
				player.bassboost = false;
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.desativado.replace('%f', 'bassboost')}`);
			}
		}
	}
};
