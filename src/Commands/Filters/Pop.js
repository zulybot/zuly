module.exports = class StopCommand {
	constructor () {
		return {
			...
			pt: {
				nome: 'pop',
				categoria: '🎵 » Filtros',
				desc: 'Ativa o filtro pop.'
			},
			en: {
				nome: 'pop',
				categoria: '🎵 » Filters',
				desc: 'Activates the pop filter.'
			},
			fr: {
				nome: 'pop',
				categoria: '🎵 » Filtres',
				desc: 'Active le filtre pop.'
			},
			/*
	...
	async run (ctx) {
		...
		if (!player) {
			...
			if (player.pop === false) {
				player.pop = true;
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.ativado.replace('%f', 'pop')}`);
			}
			if (player.pop === true) {
				player.pop = false;
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.desativado.replace('%f', 'pop')}`);
			}
		}
	}
};
