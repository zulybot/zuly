	...
	constructor () {
		return {
			...
			pt: {
				nome: 'tremolo',
				categoria: '🎵 » Filtros',
				desc: 'Ativa o filtro tremolo.'
			},
			en: {
				nome: 'tremolo',
				categoria: '🎵 » Filters',
				desc: 'Activates the tremolo filter.'
			},
			fr: {
				nome: 'tremolo',
				categoria: '🎵 » Filtres',
				desc: 'Active le filtre tremolo.'
			},
			/*
	...
	async run (ctx) {
		...
		if (!player) {
			...
			if (player.tremolo === false) {
				player.tremolo = true;
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.ativado.replace('%f', 'tremolo')}`);
			}
			if (player.tremolo === true) {
				player.tremolo = false;
				return ctx.message.channel.slashReply(`✅ ${ctx.message.author.mention} **|** ${ctx.idioma.filters.desativado.replace('%f', 'tremolo')}`);
			}
		}
	}
};
