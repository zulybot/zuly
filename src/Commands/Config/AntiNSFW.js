module.exports = class LangCommand {
	...
	async run (ctx) {
		...
		if (nsfw) {
			...
			return ctx.message.channel.slashReply(`:x: ${ctx.message.author.mention} **|** ${ctx.idioma.nsfw.desativado}`);
		}
		else {
			await global.db.set(`nsfw-${ctx.message.guildID}`, true);
			return ctx.message.channel.slashReply(`:x: ${ctx.message.author.mention} **|** ${ctx.idioma.nsfw.ativado}`);
		}
	}
};
